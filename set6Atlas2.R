...
context <- atCounts[,rownames(atInfo)[which(atInfo$ShortenedLifeStage3=="Trophozoite" | atInfo$ShortenedLifeStage3=="Schizont" | atInfo$ShortenedLifeStage3=="Ring" | atInfo$ShortenedLifeStage3=="Merozoite" | atInfo$ShortenedLifeStage3=="EEF" | atInfo$ShortenedLifeStage3=="Female" | atInfo$ShortenedLifeStage3=="Male" | atInfo$ShortenedLifeStage3=="Ookinete" | atInfo$ShortenedLifeStage3=="Oocyst" | atInfo$ShortenedLifeStage3=="bbSpz" | atInfo$ShortenedLifeStage3=="sgSpz")]]

setOok <- set6[,c(4:12,16:24)]
names(setOok) <- colnames(set6)[c(4:12,16:24)]
setGam <- set6[,c(1:3,13:15)]
names(setGam) <- colnames(set6)[c(1:3,13:15)]
laterNames <- c(names(setOok), names(setGam))

intData <- cbind(context, setOok, setGam)

#reading into Seurat objects
intData <- CreateSeuratObject(counts = intData, project = "intData")

intData <- NormalizeData(intData)
intData <- FindVariableFeatures(intData, selection.method = "vst", nfeatures = 2000)
...
intData <- RunUMAP(intData, dims = 1:26, verbose = FALSE)
intData <- FindNeighbors(intData, dims = 1:26, verbose = FALSE)
...
intData@meta.data$ShortLifeStage3[1788:1811] <- laterNames

#THEN FIX THESE
class1 <- intData@meta.data$ShortLifeStage3
class1[1806:1811] <- "SET6-GAM"
class1[1788:1805] <- "SET6-OOK"

class2 <- intData@meta.data$ShortLifeStage3
class2[1806:1808] <- "SET6-KO.GAM"
class2[1809:1811] <- "SET6-WT.GAM"
class2[1788:1790] <- "SET6-KO.OOK08"
class2[1791:1793] <- "SET6-KO.OOK16"
class2[1794:1796] <- "SET6-KO.OOK24"
class2[1797:1799] <- "SET6-WT.OOK08"
class2[1800:1802] <- "SET6-WT.OOK16"
class2[1803:1805] <- "SET6-WT.OOK24"
...
class3[c(1788:1796, 1806:1808)] <- "KO"
class3[c(1797:1805, 1809:1811)] <- "WT"
intData <- AddMetaData(intData, class3, col.name="class3")

class4 <- intData@meta.data$class2
class4[1788:1811] <- "set6"
...
png(fn, width=1000, height=1000)
thingy <- DimPlot(object=onlySet, dims = c(x1, x2), reduction = "pca", pt.size=4, label=T, cols=subCols)
print(thingy)
dev.off()
